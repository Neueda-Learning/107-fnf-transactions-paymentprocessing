...
public class PaymentService {
    ...
    public PaymentService(PaymentRepository paymentRepository, VendorRepository vendorRepository, InvoiceRepository invoiceRepository) {
    ...
    public Payment createPayment(CreatePaymentRequest request) {
        ...
        if (request.getSenderAccount().equalsIgnoreCase(request.getReceiverAccount())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Sender account and receiver account must be different");
        }
        ...
        // Go to vendor table using vendorId from invoice and check receiver account matches vendor bank account
        Vendor vendor = vendorRepository.findById(invoice.getVendor().getId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Vendor not found"));

        if (vendor.getBankAccount() == null || !vendor.getBankAccount().equalsIgnoreCase(request.getReceiverAccount())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
